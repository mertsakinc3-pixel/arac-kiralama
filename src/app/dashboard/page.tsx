...
import { IoCarSport, IoCalendar, IoList, IoPerson, IoMail, IoCall, IoLocation, IoShield, IoTrophy } from "react-icons/io5";
...
const mockLeaderboard = [
  { id: 1, rank: 1, companyName: "Elit Rent A Car", district: "Beşiktaş", bidAmount: 500 },
  { id: 2, rank: 2, companyName: "Lüks Araç Kiralama", district: "Beşiktaş", bidAmount: 400 },
  { id: 3, rank: 3, companyName: "Premium Kiralama", district: "Beşiktaş", bidAmount: 300 },
  { id: 4, rank: 4, companyName: "Hızlı Rent A Car", district: "Beşiktaş", bidAmount: 200 },
  { id: 5, rank: 5, companyName: "Ekonomik Kiralama", district: "Beşiktaş", bidAmount: 100 },
];
...
export default function DashboardPage() {
  ...
  const getRankColor = (rank: number) => {
    switch(rank) {
      ...
      case 4: return "from-slate-400 to-slate-500";
      case 5: return "from-slate-400 to-slate-500";
  ...
  const getRankBadgeColor = (rank: number) => {
    switch(rank) {
      case 1: return "bg-linear-to-br from-amber-400 to-amber-600 text-white shadow-md";
      case 2: return "bg-linear-to-br from-slate-300 to-slate-500 text-white shadow-sm";
      case 3: return "bg-linear-to-br from-orange-400 to-orange-600 text-white shadow-sm";
      default: return "bg-linear-to-br from-slate-400 to-slate-600 text-white shadow-sm";
    }
  };
          ...
          <div className="bg-linear-to-r from-slate-50 to-slate-100 p-6 border-b-2 border-slate-200">
            <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-bold text-slate-800">İlçe Lideri</h2>
                  <p className="text-slate-600 text-sm">En yüksek teklif veren kazanır!</p>
                </div>
              </div>
              
              {/* Geri Sayım - Modern */}
              <div className="bg-white border-2 border-slate-200 rounded-lg px-4 py-3 shadow-sm">
                <p className="text-slate-600 text-xs font-medium mb-1 text-center uppercase tracking-wide">Süre Bitimine</p>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex-1">
                  <p className="text-slate-800 text-sm font-semibold mb-1">
                    ⚡ Sadece ilk 5 firma görünür! En yüksek teklif veren lider olur.
                  </p>
                  <p className="text-slate-600 text-xs">
                    Başkaları daha yüksek teklif verirse sıralama anında değişir!
                  </p>
                </div>
                <button
            ...
            {showRules && (
              <div className="mb-4 bg-linear-to-br from-green-50 to-emerald-50 border-2 border-green-200 rounded-lg p-5 shadow-md animate-fadeIn">
                ...
                <div className="space-y-2 text-sm text-green-900">
                  <p><strong>1️⃣ İlk 5 Sıra:</strong> Her ilçede sadece ilk 5 firma müşterilere gösterilir.</p>
                  <p><strong>2️⃣ Dinamik Sıralama:</strong> En yüksek teklif veren her zaman üstte. Birisi daha fazla verirse sıralama değişir.</p>
                  <p><strong>3️⃣ Çıkma Riski:</strong> 5&apos;ten fazla firma daha yüksek teklif verirse, en düşük teklifli listeden çıkar!</p>
                  <p><strong>4️⃣ 7 Günlük Dönem:</strong> Her hafta yeni dönem başlar, teklifler sıfırlanır.</p>
                </div>
                <div className="mt-3 bg-green-100 border-2 border-green-300 rounded-lg p-3">
                  <p className="text-green-900 font-bold text-sm">
                    ⚡ Kartınız her an ilk 5&apos;ten çıkabilir! Teklifinizi güncel tutun.
                  </p>
                </div>
              </div>
            )}
              ...
              {mockLeaderboard.map((company) => (
                <div
                  ...
                  className={`bg-white rounded-lg border-2 overflow-hidden transform transition-all duration-200 hover:scale-105 hover:shadow-xl ${
                    company.rank === 1 ? 'border-amber-400 shadow-lg' : 'border-slate-200 shadow-md'
                  }`}
                >
                  {/* Sıra Rozeti */}
                  <div className={`bg-linear-to-br ${getRankColor(company.rank)} p-4 relative`}>
                    <div className="flex items-center justify-between mb-2">
                      <div className={`w-10 h-10 ${getRankBadgeColor(company.rank)} rounded-full flex items-center justify-center text-lg font-bold`}>
                        {company.rank}
                      </div>
                      {company.rank === 1 && (
                        <IoTrophy className="text-white drop-shadow-md" size={22} />
                      )}
                    </div>
                    <div className="text-white text-xl font-bold drop-shadow-sm">{company.bidAmount} TL</div>
                    <div className="text-white/90 text-xs font-medium">Teklif</div>
                  </div>
                  ...
                  <div className="p-4 bg-white">
                    ...
                    <div className="flex items-center justify-center text-xs mb-3">
                      <IoLocation className="text-slate-500 mr-1" size={14} />
                      <span className="text-slate-600 text-xs font-medium">{company.district}</span>
                    </div>

                    {/* Satın Al Butonu */}
                    <button
                      onClick={() => handlePurchaseRank(company.rank, company.bidAmount)}
                ...
                <Link href="/dashboard/arac-ekle">
                  <div className="group cursor-pointer bg-linear-to-br from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 rounded-xl p-8 text-white transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="text-sm text-slate-100">Yeni araç ekleyin ve kiralama havuzunuzu genişletin</p>
                    </div>
                  </div>
                </Link>
                ...
                <Link href="/dashboard/rezervasyonlar">
                  <div className="group cursor-pointer bg-linear-to-br from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 rounded-xl p-8 text-white transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="text-sm text-slate-100">Müşteri rezervasyonlarını görüntüleyin ve yönetin</p>
                    </div>
                  </div>
                </Link>
                ...
                <Link href="/dashboard/arac-listesi">
                  <div className="group cursor-pointer bg-linear-to-br from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 rounded-xl p-8 text-white transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="text-sm text-slate-100">Tüm araçlarınızı görüntüleyin ve düzenleyin</p>
                    </div>
                  </div>
                </Link>
                ...
                <Link href="/dashboard/sigorta">
                  <div className="group cursor-pointer bg-linear-to-br from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 rounded-xl p-8 text-white transition-all duration-300 transform hover:scale-105 shadow-lg hover:shadow-xl">
                    <div className="flex flex-col items-center text-center">
                      ...
                      <p className="text-sm text-slate-100">Araçlarınızın sigorta bilgilerini yönetin ve düzenleyin</p>
                    </div>
                  </div>
                </Link>
              </div>
